...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE:
		...
		window->buffer_bitmap_info.bmiHeader.biSize = sizeof(window->buffer_bitmap_info.bmiHeader);
		window->buffer_bitmap_info.bmiHeader.biWidth = window->getWidth();
		window->buffer_bitmap_info.bmiHeader.biHeight = window->getHeight();
		window->buffer_bitmap_info.bmiHeader.biPlanes = 1;
		window->buffer_bitmap_info.bmiHeader.biBitCount = 32;
		window->buffer_bitmap_info.bmiHeader.biCompression = BI_RGB;
	}
...
void Window::SetPixelRGB(unsigned int x, unsigned int y, unsigned int color) {
	unsigned int* pixel = (unsigned int *) buffer_memory + x + (y * this->width);
	*pixel = color;
}
